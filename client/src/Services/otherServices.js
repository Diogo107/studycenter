...
const updateProfile = async (data) => {
	console.log('this is the client', data);
	const {
		id,
		name,
		email,
	...
	try {
		const building = await instance.post('/updateProfile', {
			id,
			name,
			email,
			admin,
...
const updateCart = async (data) => {
	console.log('this is the client', data);
	const { id, cart } = data;
	console.log('(other Services) updateCart', {
		id,
		cart,
	});
	try {
		const building = await instance.post('/updateCart', {
			id,
			cart,
		});
		return building;
